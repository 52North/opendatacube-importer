...
# Manuals:
# - https://catalogue.marine.copernicus.eu/documents/PUM/CMEMS-GLO-PUM-001-027.pdf (waves)
# - https://catalogue.marine.copernicus.eu/documents/PUM/CMEMS-GLO-PUM-001-024.pdf (physics,currents)
# Dataset cmems_mod_glo_phy_anfc_0.083deg_PT1H-m
# contains the hourly mean surface fields: potential temperature, currents and surface sea surface level information.
...
class CmemsPhysicsLoader(BasicLoader):
    """
    ...
    def __init__(self):
        super().__init__()
        # self.url = os.getenv(
        # 'CMEMS_WAVES_URL', 'https://uni-bonn.sciebo.de/s/6wrgdIndjpfRJuA/download')
        # self.zip_file = os.getenv('CMEMS_WAVES_ZIP', 'anthroprotect.zip')
        self.folder = os.getenv('CMEMS_WAVES_FOLDER', 'physics')
        # self.chunk_size = 8192  # in bytes
        # self.force_download = os.getenv('CMEMS_WAVES_FORCE_DOWNLOAD', False)
        # self.zip_sha256_hash = os.getenv(
        #     'CMEMS_WAVES_ZIP_SHA256', '88ab511c2c89b64cd29d5f0d03174b450af3dd66a48423bc7f212f4391206f0d')

        product_names = os.getenv('CMEMS_WAVES_PRODUCT_NAMES')
        if product_names:
            self.product_names = product_names.split(' ')
        else:
            self.product_names = ['physics']
    ...
    def create_product_dataset_map(self, global_data_folder):
        ...
        # for idx, subfolder in enumerate(['physics']):
    ...
    def create_product_metadata_eo3(self, odc_product_name):
        ...
        # if odc_product_name.startswith('physics'):
        #     measurements = physics_measurements
        # else:
        #     logger.error(
    ...
    def create_dataset_metadata_eo3(self, odc_product_name, dataset):
        ...
        # if odc_product_name.startswith('waves'):
        #     measurements = dict.fromkeys(list(physics_measurements.keys()))
        #     platform = None
        #     instrument = None
        # else:
        #     logger.error(
        #         "No mwasurements information found for product '{}'".format(odc_product_name))

        metadata = self._create_metadata_document(odc_product_name)

        # Extract geo-information from file
        # Read the the dataset with decode_coords='all' to get crs
        df_nc = xr.open_dataset(dataset, decode_coords='all')
        ...
        measurement_dict = {}
        for var in physics_measurements.keys():
